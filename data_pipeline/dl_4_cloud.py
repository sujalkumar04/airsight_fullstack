"""
Script 4: Download Cloud Cover — 2° global grid, 2015-2022
Run: python3 dl_4_cloud.py
"""
...
OUTPUT = os.path.join(BASE_DIR, 'data', 'cloud_global.csv')

if os.path.exists(OUTPUT):
    print("⏭️  Cloud Cover already exists, skipping."); exit()
...
print(f"☁️  Cloud Cover: {n_pts} points, {len(chunks)} chunks")

cloud = ee.ImageCollection("MODIS/061/MOD08_M3") \
    .filterDate(START_DATE, END_DATE) \
    .select('Cloud_Fraction_Mean_Mean')
images = cloud.toList(cloud.size())
n = cloud.size().getInfo()
...
for i in range(n):
    ...
    for c in chunks:
        try:
            ...
            for f in r['features']:
                p = f['properties']
                rows.append({'date': d, 'lat': p['lat'], 'lon': p['lon'], 'cloud_fraction': p.get('mean')})
...
print(f"\n✅ Cloud done: {len(df):,} rows | {(time.time()-t)/60:.1f} min")
